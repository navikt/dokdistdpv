package no.nav.dokdistdpv.utils;

public class AltinnConstant {
	public static final String LANGUAGE_CODE_BOKMAAL = "1044";
	public static final String FROM_ADDRESS = "ikke-besvar-denne@nav.no";
	public static final String NOTIFICATION_UTEN_REVARSEL = "VarselDPVUtenRevarsel";
	public static final String NOTIFICATION_MED_REVARSEL = "VarselDPVMedRevarsel";

	public static final String MESSAGE_TITLE_VEDTAK = "Vedtak fra NAV";
	public static final String MESSAGE_TITLE_VIKTIG = "Viktig brev fra NAV";
	public static final String MESSAGE_TITLE_ANNET = "Melding fra NAV";

	public static final String VEDTAK = "vedtak";
	public static final String VEDTAKET = "Vedtaket";
	public static final String VIKTIG_BREV = "viktig brev";
	public static final String BREVET = "Brevet";
	public static final String MELDING = "melding";
	public static final String MELDINGEN = "Meldingen";
}
